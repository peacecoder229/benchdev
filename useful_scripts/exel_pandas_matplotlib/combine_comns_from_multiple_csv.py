...
def get_opts():
    ...
    parser.add_option("--inputfiles", dest="infile", help="provide Input file names and cols in format \"file1:cols,col2,col3 file2:col1:col2:col3 \"", default=None)
...
#READ all input files and combine given columns into a single dataframe df
    df = pd.DataFrame()
    csvinfo = re.split("\s+", csv)
    print(csvinfo)
    for info in csvinfo:
        csvfile, selcols = info.split(":")
        selcols = [c for c in selcols.split(",")]
        dftmp = pd.read_csv(csvfile, sep=",", header=0)
        for c in selcols:
            df[c] = dftmp[c]
