...
def read_datafile(path, name, out):
    print("begin to processing %s" %name)
    fname = os.path.join(path, name)

    df = pd.read_csv(fname, names=["ts","th_limit", "budget", "delta", "rpq_occ"], index_col=False)
...
def plot_ewma(df, filename, path):
    fig = plt.figure(filename)
    ...
    text = "kp=%s Ki=%s Kd=%s sp=%s ewma=%s" %(p,i,d,sp,ema)
    plt.figtext(0.35, 0.95, text)
    savepng = "%s/%s" %(path, filename)
    plt.savefig(savepng, dpi=300, transparent=True,pad_inches=0)
def summary_data(df):
