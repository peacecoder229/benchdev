...
)
{
    ...
    struct stat st;
    int i;
    unsigned long long begin_tsc, diff_tsc;
    int num_pages;
    int bytes_to_read = 0;

    if (argc < 2){
        printf("usage: %s filename bytes_to_read<>\n", argv[0]);
        return -1;
    }
    ...
    num_pages = bytes_to_read/4096;
    begin_tsc = __rdtsc();
    for (i=0; i<num_pages; i++) {
	memcpy(buf, file_map_addr, 4096);
	file_map_addr += 4096;
    }
    diff_tsc = __rdtsc() - begin_tsc;
    diff_tsc /= 1000;

    printf("Read %d pages, elapsed k cycles = %d\n", num_pages, diff_tsc);
