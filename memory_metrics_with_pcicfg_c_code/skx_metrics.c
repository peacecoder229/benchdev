...
#define INTERVAL	100000  /* sampling rate in microsecond, this is an example: 1s */
...
double avg_metrics[SOCK_NUM][METRICS];

int cpu_time0[CPU_TIME_NUM];
int cpu_time1[CPU_TIME_NUM];
int cpu_time_diff[CPU_TIME_NUM];
...
static void calculate_cpu_util(void)
{
    int i;
    float util;
    int total_time = 0;
    int non_idle;
    memcpy(cpu_time0,cpu_time1,sizeof(int)*CPU_TIME_NUM);
    get_cpu_time();
    for(i=0; i < CPU_TIME_NUM; i++)
    {
        cpu_time_diff[i] = cpu_time1[i]-cpu_time0[i];
        total_time += cpu_time_diff[i];
    }
    non_idle = total_time - cpu_time_diff[3];
    util = (float)non_idle/(float)(total_time)*100;
    printf("%f,\t", util);
}
...
void init(void)
{
	...
	//set_event(RPQ_INSERTS_IND, 0x00440020);  /* WPQ_INSERTS */
	//set_event(RPQ_OCCUPANCY_IND, 0x00440081);  /* WPQ_OCCUPANCY */
	...
	write_each_imc(0xF4, 0x00000000);
        memset((int *)cpu_time0, 0, sizeof(int)*CPU_TIME_NUM);
        memset((int *)cpu_time1, 0, sizeof(int)*CPU_TIME_NUM);
        memset((int *)cpu_time_diff, 0, sizeof(int)*CPU_TIME_NUM);
}
...
*/
        ...
        printf("tsc,\t\tutil\t\ts0_MB,\ts0_RPQ_latency,\ts0_RPQ_occupancy,\ts1_MB,\ts1_RPQ_latency,\ts1_RPQ_occupancy\n");
	for ( ; ; ) {
            int avg_n = 0;
            memset((void *)avg_metrics, 0, sizeof(avg_metrics));
            reset_counting();
            read_tsc(); 
            calculate_cpu_util();
            for (avg_n = 0; avg_n < AVG_WINDOW; avg_n++){
		...
		occupancy(delta_time);
                for (i = 0; i < SOCK_NUM; i++) {
                    avg_metrics[i][METRIC_MEM_BW] += metrics[i][METRIC_MEM_BW];
                    avg_metrics[i][METRIC_AVG_LAT] += metrics[i][METRIC_AVG_LAT];
                    avg_metrics[i][METRIC_OCCUPANCY] += metrics[i][METRIC_OCCUPANCY];
                }
             }
		 ...
		 */
		for (i = 0; i < SOCK_NUM; i++) {
                        avg_metrics[i][METRIC_MEM_BW] = avg_metrics[i][METRIC_MEM_BW]/AVG_WINDOW;
                        avg_metrics[i][METRIC_AVG_LAT] = avg_metrics[i][METRIC_AVG_LAT]/AVG_WINDOW;
                        avg_metrics[i][METRIC_OCCUPANCY] = avg_metrics[i][METRIC_OCCUPANCY]/AVG_WINDOW;
			printf("%lf,\t%lf,\t%lf", (double)(avg_metrics[i][METRIC_MEM_BW]), (double)avg_metrics[i][METRIC_AVG_LAT], (double)avg_metrics[i][METRIC_OCCUPANCY]);
			if (!i)
				printf(",\t");
			else
				printf("\n");
