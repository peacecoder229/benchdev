...
static unsigned long rdmsr_ser_bmk(unsigned long addr, int iter){
	...
	printk("rdmsrbmk: run_rdmsr_test: 0x%x=0x%16.16lx: iter %d diff = %lu\n",(unsigned int)addr,(unsigned long long)((hi<<32)|lo),iter,((unsigned long)(eax2-eax1)));
	return 0;
}


static unsigned long rdpmc_ser_bmk(unsigned long ctr, int iter){
        register unsigned long eax1, edx1; 
        register unsigned long eax2, edx2; 
	//unsigned long ecx; 
        register unsigned long lo=0, hi=0;
	...
	printk("rdmsrbmk: run_rdpmc_test: 0x%x=0x%16.16lx: iter %d diff = %lu\n",(unsigned int)ctr,(unsigned long long)((hi<<32)|lo),iter,((unsigned long)(eax2-eax1)));
	return 0;
}
...
static void run_rdpmc_ser_test(unsigned long addr){
        int idx = 0;
	printk("rdmsrbmk: Running run_rdpmc_test for 0x%x...\n",(unsigned int)addr);

	for(idx=0; idx<ITER; ++idx){
                rdpmc_ser_bmk(addr,idx);
        }

	printk("rdmsrbmk: run_rdpmc_test completed.\n");
	return;
...
static int run_test(void){
	...
	//program fixed perf counters to enable ctr0(inst_ret.any  and ctr1 unhalted_cycles.core
        wrmsr_ser_bmk(0x38D,0x33,0x33,1);
        unsigned long val = 0x3003C;
        unsigned long evval = ((val)|=(1<<22));
        //gen counter programmed to count ctr1 unhalted_cycles.core
        wrmsr_ser_bmk(0X186,evval,evval,1);
        rdmsr_ser_bmk(0X186,1);
        unsigned long c = (1<<30); 
        //rpmc used to read counter 0
         rdpmc_ser_bmk(0,1);
        //rdmsr used to read counter 0
        rdmsr_ser_bmk(0xC1,1);
        // use rdpmc to read fixed counter1 to read ulhalted cycles core
        unsigned long fxctr = (1<<30)|1;
        rdmsr_ser_bmk(0x30A,1);
        rdmsr_ser_bmk(0xC1,1);
        rdmsr_ser_bmk(0x30A,1);
	rdpmc_ser_bmk(fxctr,1);
        //run_rdmsr_ser_test(0xC1);
	//run_rdpmc_ser_test(0);
        //run_rdpmc_ser_test(0xC1);
