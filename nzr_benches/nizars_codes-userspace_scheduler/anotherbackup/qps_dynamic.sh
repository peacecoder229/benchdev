...
cgexec -g cpu:lesscpulimited /pnpdata/clucene_benchmark_new/src/benchmark-dev/clucene_search_main/./B_new_script.sh >/root/scheduler/outfile50 &
...
for((i=0;i<300;i++ ))
	...
	qps=$(tail -n 1 outfile50 | egrep -o "^[0-9]{1,7}\.{0,1}[0-9]{1,7}" )
	qps_diff=$(echo "0.01*$period*$qps_old -0.01*$period*$qps " | bc )
	qps_old=$qps
