...
int trace_second_func_entry(struct pt_regs *ctx)
{
    ...
    bpf_trace_printk("second entry , %d\\n",pid );
    FILTER
    ENTRYSTORE
    start2.update(&pid, &ts);
...
int trace_second_func_return(struct pt_regs *ctx)
{
    ...
    u32 pid = pid_tgid;
    u32 tgid = pid_tgid >> 32;
    bpf_trace_printk("second return , %d\\n",pid );
    // calculate delta time
    tsp = start2.lookup(&pid);
    if (tsp == 0) {
        return 0;   // missed start
    }
    delta2 = bpf_ktime_get_ns() - *tsp;
    start2.delete(&pid);
    FACTO2
...
int trace_func_entry(struct pt_regs *ctx)
{
    ...
    u32 tgid = pid_tgid >> 32;
    u64 ts = bpf_ktime_get_ns();
    bpf_trace_printk("first entry , %d\\n",pid );
    FILTER
    ENTRYSTORE
    start.update(&pid, &ts);
...
int trace_func_return(struct pt_regs *ctx)
{
    ...
    u32 pid = pid_tgid;
    u32 tgid = pid_tgid >> 32;
    bpf_trace_printk("first return , %d\\n",pid );
    // calculate delta time
...
if args.milliseconds:
    ...
    bpf_text = bpf_text.replace('FACTO2', 'delta2 /= 1000000;')
    label = "msecs"
elif args.microseconds:
    bpf_text = bpf_text.replace('FACTOR', 'delta /= 1000;')
    bpf_text = bpf_text.replace('FACTO2', 'delta2 /= 1000;')
    label = "usecs"
else:
    bpf_text = bpf_text.replace('FACTOR', '')
    bpf_text = bpf_text.replace('FACTO2', '')
    label = "nsecs"
if need_key:
    bpf_text = bpf_text.replace('STORAGE', 'BPF_HASH(ipaddr, u32);\n' +
        'BPF_HISTOGRAM(dist, hist_key_t);')
    bpf_text = bpf_text.replace('STORAG2', 'BPF_HASH(ipaddr, u32);\n' +
        'BPF_HISTOGRAM(dist2, hist_key_t);')
    ...
    bpf_text = bpf_text.replace('STORAG2', 'BPF_HISTOGRAM(dist2);')
    bpf_text = bpf_text.replace('ENTRYSTORE', '')
    bpf_text = bpf_text.replace('STORE',
        'dist.increment(bpf_log2l(delta));')
    bpf_text = bpf_text.replace('STOR2',
        'dist2.increment(bpf_log2l(delta2));')
...
if not library:
    ...
    b.attach_kprobe(event_re=args.E, fn_name="trace_second_func_entry")
    b.attach_kretprobe(event_re=args.E, fn_name="trace_second_func_return")
    ...
    b.attach_uprobe(name=library, sym_re=args.E, fn_name="trace_second_func_entry",
                    pid=args.pid or -1)
    b.attach_uretprobe(name=library, sym_re=args.E,
...
while (1):
    ...
    if need_key:
        ...
        dist2.print_log2_hist(label, "Function", section_print_fn=print_section,
            bucket_fn=lambda k: (k.ip, k.pid))
    else:
        dist.print_log2_hist(label)
        dist2.print_log2_hist(label)
    dist.clear()
    dist2.clear()
    if exiting:
        print("Detaching...")
