...
/*
static inline unsigned long nopnop(void){
        unsigned long res=0;
...
static inline unsigned long rdtsc_bmk(void){
...
/*
static inline unsigned long rdmsr64(unsigned long addr){
...
/*
static inline void wrmsr64(unsigned long addr, unsigned long val){
...
static int __init rdmsrbmk_init(void){
	...
	printk("rdmsrbmk: Initializing kernel module...\n");	
		...
		if(!IS_ERR(ts)){
			...
			printk("We are using use_kthread value %d\n", compiled);
		}
		...
		trace_printk("rdmskbmk: Kernel says we are allowde to run on CPU: %u.\n",cpu);
	#else
		retval = run_test();
	#endif
