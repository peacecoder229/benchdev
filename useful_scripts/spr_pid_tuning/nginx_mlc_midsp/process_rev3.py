...
def read_datafile(name, out, figlist, color):
    print("begin to processing %s" %name)

    df = pd.read_csv(name, names=["ts","pid_out", "budget", "ewma_delta", "rpq_occ"], index_col=False)
...
def main(path="."):
    ...
    for f in filelist:
        i = i %7
        read_datafile(f, out, figlist, colortop[i])
        i += 1
