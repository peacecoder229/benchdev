...
#Note... this script reads in all .csv files  all columns are added from gives names list. Thiscold be also done by hvaing header=None as shown in the 
#script plt_avg.py
...
def plot_pid(files):
  for k in range(0, len(files), 14):
    ...
    for name in grp:
       #print("begin to processing %s" %name)
       #Above file group pf 14 files is created so that only 14 could be meaningfully fit on a plot. last grp get remainig few files
...
# All functions were not getting the outfile handle.. so
#below way file was opened inside main and
#passed to the function for appending..or just writing
