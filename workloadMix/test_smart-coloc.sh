...
#	cat `find $p -name summary.txt ` | awk '{print $6}'
 	awk -F: '{print $2}' $p/hp/summary.txt | awk '{print $1}'
...
{	
	...
	initial_settings
	score=$(read_hp_after_finish $hp_workload "empty" $ROOT/hp)
...
# 	enable_HWDRC 
	...
	do
		...
		score=$(read_hp_after_finish $hp_workload $lp_workload $ROOT/LOOP-$LOOP)
	...
	do
		...
		score=$(read_hp_after_finish $hp_workload $lp_workload $ROOT/HWDRC-LOOP-$LOOP)

		if [  $( echo "$taget_score < $score" | bc ) -eq 1  ]
		then
			LOOP=100
			echo LP core: $LP_INSTANCE, HP score: $score
		else
