...
void searcher_contents(string buf, int fd) {
  ...
  string terms_str = buf.substr(buf.find(" **") + 3, buf.substr(buf.find(" **") + 3).length() - buf.substr(buf.rfind(" **")).length()); // get term
    ...
    send(fd, sendToClient.str().c_str(), sendToClient.str().length(), 0);
...
void process_new_data(int fd)
{
	...
	while ((count = read(fd, buf, sizeof(buf) - 1))) {
        ...
        string queryString1 = queryString + " SID" + std::to_string(fd);
        searcher_contents(queryString1, fd);
	}
...
int main(int argc, char* argv[])
{
    ...
    while(1) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if ((currentTime - startTime).seconds() > TEST_TIME_LEN) {
                sleep(8);
                completeFlag = true;
                break;
            }
