...
def clean_env():
    try:
        ...
        cmd = "docker ps -a | grep -E \'stream|speccpu\' | awk \'{print $1}\' | xargs docker rm -f"
        os.system(cmd)
    except:
        print "err"
...
def start_redis(cores, port):
    ...
    c_cmd = 'taskset -c %s memtier_benchmark -s localhost -p %s --pipeline=30 -c 10 -t 10 -d 1000 --key-maximum=42949 --key-pattern=G:G --key-stddev=1177 --ratio=1:1 --distinct-client-seed --randomize --test-time=18000 --run-count=1 --hide-histogram &>redis_output/redis%s.log &' % (
    hp_cores, port, port)
    os.system(s_cmd)
    os.system(c_cmd)
...
def run_test():
    ...
    start_speccpu(LP_cores, LP_NUM, 'bwaves')
